...
// Caminho do arquivo de cookies exportado (formato Netscape .txt) de uma conta logada do YouTube.
const COOKIES_PATH = path.resolve(__dirname, '..', 'database', 'yt-cookies.txt');

// Converte o formato Netscape cookies.txt (domain, flag, path, secure, expiration, name, value)
// para o array de objetos que ytdl.createAgent() espera.
function parseNetscapeCookies(raw) {
    return raw
        ...
        .map(line => {
            ...
            const [domain, flag, cpath, secure, expiration, name, value] = parts;
            const exp = parseInt(expiration, 10) || 0;
            return {
                domain,
                hostOnly: flag !== 'TRUE',
                path: cpath,
...
try {
    if (fs.existsSync(COOKIES_PATH)) {
        ...
        console.log(`✅ [play.js] ${cookies.length} cookies do YouTube carregados, autenticando requisições.`);
    } else {
        console.warn('⚠️ [play.js] yt-cookies.txt não encontrado — requisições ao YouTube sem autenticação têm grande chance de cair no bloqueio "Sign in to confirm you\'re not a bot".');
    }
} catch (e) {
    console.error('❌ [play.js] Falha ao carregar yt-cookies.txt:', e.message);
}
    ...
    async execute(client, msg, { chatId, args, User, senderRaw }) {
        ...
        try {
            ...
            const mensagem = isBotBlock
                ? `❌ O YouTube bloqueou a requisição por suspeita de bot. ${ytdlAgent ? 'Os cookies configurados podem ter expirado — é necessário gerar um novo yt-cookies.txt.' : 'Configure um arquivo de cookies (yt-cookies.txt) para autenticar as requisições — veja instruções com o desenvolvedor.'}`
