            ...
            switch (jogo) {
                ...
                case '21': {
                    ...
                    if (isNaN(alvo) || alvo < 2 || alvo > 21) {
                        if (!isComandante) await User.updateOne({ userId: senderId, groupId: chatId }, { $inc: { casinoCount: -1 } });
                        return await client.sendMessage(chatId, "🃏 Escolha um alvo entre 2 e 21!", { sendSeen: false });
                    }
                    ...
                    if (seuPonto === alvo) {
                        ...
                        await User.updateOne({ userId: senderId, groupId: chatId }, { $inc: { coins: -valorAp } });
                        await client.sendMessage(chatId, `🃏 *PERDEU!* Tirou ${seuPonto}. -${valorAp.toLocaleString()} YC.`, { mentions: [senderId], sendSeen: false });
                    }
                    break;
                }
